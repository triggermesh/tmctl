...
func NewCmd() *cobra.Command {
	...
	dumpCmd := &cobra.Command{
		Use:     "dump [broker]",
		Short:   "Generate manifest for different Platforms",
		Example: "tmctl dump",
	...
	dumpCmd.Flags().StringVarP(&o.Platform, "platform", "p", "kubernetes", "Platform")
...
func (o *dumpOptions) dump() error {
	...
	doServices := []*godo.AppServiceSpec{}
			...
			if container, ok := component.(triggermesh.Runnable); ok {
				if _, err := container.Info(context.Background()); err == nil {
					if platform, ok := component.(triggermesh.Platform); ok {
						...
						switch o.Platform {
						case "digitalocean":
							if doService, err := platform.AsDigitalOcean(secretsEnv); err == nil {
								doServices = append(doServices, doService)
							}
						case "docker-compose":
							if composeService, err := platform.AsDockerComposeObject(secretsEnv); err == nil {
